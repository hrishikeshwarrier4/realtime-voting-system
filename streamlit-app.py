...
def plot_colored_barchart(results):
...
def plot_colored_piechart(data):
...
def update_data():
    ...
    #fetch votoing statistics from postgres
    ...
    #Displaying Statistics and Visualization
    st.markdown("""----""")
    ...
    with col1:
        bar_fig = plot_colored_barchart(results)
        st.pyplot(bar_fig)

    with col2:
        pie_fig = plot_colored_piechart(results)
        st.pyplot(pie_fig)
    ...
    #Fetch data on Kafka for location
...
st.title("Real Time Voting System Dashboard")
